...
import { usedeleteFromCart, useupdateProductQuan } from "@/services/mutaions";
...
const Cart: FC<CartProps> = () => {
  ...
  const { mutate, data: quanUpdateData } = useupdateProductQuan();
  function updatequantity(id: any, count: any): void {
    mutate(id, count);
    console.log(count);
    console.log(quanUpdateData);
  }
  return (
    <>
      {isPending ? (
          ...
          {getdata?.data?.products?.length > 0 ? (
            <div className="container m-auto my-12 bg-light-color p-3">
              ...
              <div className="flex gap-1 text-main-color">
                <h3>Total Price </h3>
                <PriceFormat price={getdata?.data.totalCartPrice} />
              </div>
              {getdata?.data.products.map((product: product, index: number) => (
